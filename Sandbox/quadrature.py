...
if __name__ == "__main__":
    ...
    print(f"Adaptive1 {abs(I1 - exact):e} ({fevs1})")
    x1 = np.linspace(a, b, fevs1)
    print(f"Euler ({fevs1}): {abs(np.sum(func(x1)) * (b-a)/(fevs1-1) - exact):e}")
    print(f"Simpson ({fevs1}): {abs(scipy.integrate.simpson(func(x1), x1) - exact):e}")

    # adaptive 2
    print("")
    I2, fevs2, xs2 = adaptive2(func, a, b, n, atol, rtol, depth)
    print(f"Adaptive2 {abs(I2 - exact):e} ({fevs2})")
    x2 = np.linspace(a, b, fevs2)
    print(f"Euler ({fevs2}): {abs(np.sum(func(x2)) * (b-a)/(fevs2-1) - exact):e}")
    print(f"Simpson ({fevs2}): {abs(scipy.integrate.simpson(func(x2), x2) - exact):e}")

    x = np.linspace(a, b, 1000)
    plt.plot(x, func(x))
    plt.plot(xs1, func(xs1), '.')
    plt.plot(xs2, func(xs2), '.')
    plt.show()

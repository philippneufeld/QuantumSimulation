...
namespace QSim
{
    ...
    bool DataFile3::Open(const std::string& name, DataFile3OpenFlag flag)
    {
        ...
        if (flag & (DataFile3_MUST_NOT_EXIST | DataFile3_TRUNCATE))
        {
            ...
            m_file = H5Fcreate(name.c_str(), 
                (flag & DataFile3_MUST_NOT_EXIST) ? H5F_ACC_EXCL : H5F_ACC_TRUNC, 
                H5P_DEFAULT, H5P_DEFAULT);
        }
        else if(flag & DataFile3_MUST_EXIST)
        {
            // open existing file
    ...
    bool DataFile3::IsOpen() const
    {
        return !(m_file < 0);
    }

    DataFileGroup DataFile3::OpenRootGroup()
    {
        if (m_file < 0)
            return H5I_INVALID_HID;

...
#ifndef QSim_NLevelSystemQM2_H_
#define QSim_NLevelSystemQM2_H_
...
namespace QSim
{
    ...
    template<int N>
    class TNLevelSystemQM2 : public TNLevelSystemCRTP2<N, TNLevelSystemQM2<N>>
    {
        friend class TNLevelSystemCRTP2<N, TNLevelSystemQM2<N>>;
        using MyParent = TNLevelSystemCRTP2<N, TNLevelSystemQM2<N>>;
    public:

        // constructors
        //template<int dummy=N, typename=EnableDefaultCtor_t<dummy>>
        TNLevelSystemQM2() : MyParent() { }
        TNLevelSystemQM2(unsigned int dims) : MyParent(dims) { }
        
        // copy operations
        TNLevelSystemQM2(const TNLevelSystemQM2&) = default;
        TNLevelSystemQM2& operator=(const TNLevelSystemQM2&) = default;
    ...
    template<int N>
    Eigen::Matrix<std::complex<double>, N, N> TNLevelSystemQM2<N>::GetHamiltonianTI(
        const Eigen::Ref<const Eigen::VectorXd>& detunings, double velocity) const
    {
    ...
    template<int N>
    Eigen::Matrix<std::complex<double>, N, N> TNLevelSystemQM2<N>::GetDensityMatrixSS(
        const Eigen::Ref<const Eigen::VectorXd>& detunings, double velocity) const
    {
    ...
    template<int N>
    bool TNLevelSystemQM2<N>::PrepareCalculation()
    {
    ...
    template<int N>
    bool TNLevelSystemQM2<N>::PreparePhotonBasis(std::vector<unsigned int>& trans_path, 
    ...
    template<int N>
    Eigen::Matrix<std::complex<double>, N, N> TNLevelSystemQM2<N>::GetHamiltonianAux(
        const Eigen::Ref<const Eigen::VectorXd>& detunings, double velocity) const
    {
    ...
    template<int N>
    Eigen::Matrix<std::complex<double>, N, N> TNLevelSystemQM2<N>::GetHamiltonianFast(
        const Eigen::Matrix<std::complex<double>, N, N>& auxData, double t) const 
    { 
        return auxData; 
    }

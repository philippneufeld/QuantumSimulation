...
COLOR_PALETTE = np.array([col.to_rgba(c) for c in ("blue", "green", "red", "pink", "brown")])
...
def plot_stark_theory_lines(ax, path, color_func, xoff=0, yoff=0, unit=energyGHz):
    with h5py.File(path) as file:
        ...
        for idx, datagroup in enumerate(tqdm.tqdm([file[k] for k in keys])):
            efields[idx] = float(datagroup.attrs["Electric_Field"]) / 100 - xoff
...
if __name__ == '__main__':
    ...
    measurement_folder = os.path.join(get_miccell_folder(), "TraceGasSensing/Measurements/2022/2022-Stark-Map/Data/Voltage")
    meas_path = os.path.join(measurement_folder, "2022-07-13_15-54-46_834.91_1.30_32.00_0.10.h5")
